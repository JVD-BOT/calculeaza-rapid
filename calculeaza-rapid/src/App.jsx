        ...
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1, height: 28, background: "rgba(255,255,255,0.04)", borderRadius: 6, overflow: "hidden", position: "relative" }}>
            <div style={{
              ...
              height: "100%",
              background: item.color || "linear-gradient(90deg, #3b82f6, #60a5fa)",
              borderRadius: 6,
...
function SalaryCalc() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, background: "rgba(255,255,255,0.02)", borderRadius: 14, padding: "8px 0", marginBottom: 20, border: "1px solid rgba(255,255,255,0.05)" }}>
        <Stat label="Salariu Net" value={`${formatRON(r.net)}`} accent="#34d399" sub="în mână / lună" />
      ...
      <BarChart items={[
        { label: "CAS (25%)", value: r.cas, color: "linear-gradient(90deg, #ef4444, #f87171)" },
        { label: "CASS (10%)", value: r.cass, color: "linear-gradient(90deg, #f59e0b, #fbbf24)" },
        { label: "Impozit (10%)", value: r.impozit, color: "linear-gradient(90deg, #8b5cf6, #a78bfa)" },
        { label: "CAM (2.25%)", value: r.cam, color: "linear-gradient(90deg, #6366f1, #818cf8)" },
        { label: "Net", value: r.net, color: "linear-gradient(90deg, #10b981, #34d399)" },
...
function PFACalc() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, background: "rgba(255,255,255,0.02)", borderRadius: 14, padding: "8px 0", marginBottom: 20, border: "1px solid rgba(255,255,255,0.05)" }}>
        <Stat label="Venit Net Anual" value={`${formatRON(r.venitDupaImpozitare)}`} accent="#34d399" sub={`~${formatRON(r.venitDupaImpozitare / 12)} lei/lună`} />
        <Stat label="Total Taxe" value={`${formatRON(r.totalTaxe)}`} accent="#ef4444" sub={`${((r.totalTaxe / (r.venitNet || 1)) * 100).toFixed(1)}% rată efectivă`} />
      </div>
      <BarChart items={[
        { label: "Impozit (10%)", value: r.impozit, color: "linear-gradient(90deg, #8b5cf6, #a78bfa)" },
        { label: "CAS (25%)", value: r.cas, color: "linear-gradient(90deg, #ef4444, #f87171)" },
...
function MortgageCalc() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, background: "rgba(255,255,255,0.02)", borderRadius: 14, padding: "8px 0", marginBottom: 20, border: "1px solid rgba(255,255,255,0.05)" }}>
        <Stat label="Rată Lunară" value={`${formatRON(r.rataLunara)}`} accent="#60a5fa" sub="lei / lună" />
...
function FAQSection() {
  ...
  const faqsRO = [
    { q: "Cum calculez impozitul pe salariu în România?", a: "Impozitul pe venit în România este de 10% aplicat la baza impozabilă, după deducerea CAS (25%) şi CASS (10%). Folosiți calculatorul de mai sus pentru a afla salariul net din brut în câteva secunde, actualizat conform Codului Fiscal 2026." },
    { q: "Care este cota de impozit pe venit în România în 2026?", a: "România aplică o cotă unică de impozit pe venit de 10% pentru salarii şi majoritatea veniturilor persoanelor fizice. Angajații plătesc şi CAS 25% (pensie) şi CASS 10% (sănătate). Angajatorul plăteşte CAM (2,25%). Angajații IT cu salariu brut peste 10.000 lei sunt scutiți de impozit." },
    { q: "Cum calculez taxele pentru PFA în România?", a: "Taxele PFA în România includ impozit pe venit 10%, CAS 25% (calculat la 24× salariul minim brut anual) şi CASS 10% (calculat la minimum 6× salariul minim). Accesați tab-ul PFA de mai sus pentru un calcul complet pentru 2026." },
    ...
    { q: "Cât salariu net primesc din 5.000 lei brut în România?", a: "Pentru un salariu brut de 5.000 lei (2026), salariul net este de aproximativ 2.925 lei/lună după deducerea CAS (1.250 lei), CASS (500 lei) şi impozit (325 lei). Introduceți orice sumă în calculator pentru rezultate instant." },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e2e8f0", fontFamily: "'Geist Mono', 'Courier New', monospace", opacity: loaded ? 1 : 0, transition: "opacity 0.6s ease" }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "40px 20px 60px" }}>
        ...
        {/* Tab Navigation */}
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${tabs.length}, 1fr)`, gap: 6, marginBottom: 32, background: "rgba(255,255,255,0.02)", borderRadius: 16, padding: 6, border: "1px solid rgba(255,255,255,0.05)" }}>
          {tabs.map((t) => (
